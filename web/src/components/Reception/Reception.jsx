  ...
  useEffect(() => {
    GetAllApointment(accessToken).then((response) => {
      ...
      const nearest = response.data.reduce((nearestAppt, currentAppt) => {
        const currentDateTime = moment(`${currentAppt.date} ${currentAppt.time}`, "YYYY-MM-DD HH:mm");
        const today = moment().startOf('day'); // Получаем начало сегодняшнего дня
        if (!nearestAppt && currentDateTime.isSameOrAfter(today)) {
          return currentAppt;
        } else if (currentDateTime.isSameOrAfter(today) && currentDateTime.isBefore(moment(`${nearestAppt.date} ${nearestAppt.time}`, "YYYY-MM-DD HH:mm"))) {
          return currentAppt;
        }
        return nearestAppt;
      }, null);
  ...
  return (
    <div className={styles.Reception}>
      ...
      <div>
        {nearestAppointment && (
          <div>
            <p><strong>Дата:</strong> {nearestAppointment.date}</p>
            <p><strong>Специалист:</strong> {nearestAppointment.doctor.specialist}</p>
            <p><strong>Врач:</strong> {`${nearestAppointment.doctor.surname} ${nearestAppointment.doctor.name} ${nearestAppointment.doctor.patronymic}`}</p>
          </div>
        )}
        {!nearestAppointment && (
          <div>
              ...
              <Link to="/Client/MakeAppointment">
                  <button className={styles.buttonApoint}><img src="./../img/add.svg"/>Записаться на прием</button>
                </Link>
          </div>
